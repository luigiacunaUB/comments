...
// Constuct a new Validator and return a pointer to it
...
// Let's check to see if Validator's map contains any entries
func (v *Validator) IsEmpty() bool {
	return len(v.Errors) == 0
}

// Add a new error entry to the Validator's error map
// Check first if an entry with the same key does not exist
func (v *Validator) AddError(key string, message string) {
	_, exist := v.Errors[key]
	if !exist {
